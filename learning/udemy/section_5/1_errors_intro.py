...
"""
We’ll quickly get a NameError. That’s a built-in error type in Python that suggests we used a name (in this case, `my_variable`) 
that didn’t exist. It’s great to receive that error, because once you know what it means you can easily find the source of the problem.
...
The name of the error is very useful. But even more useful is the *stack trace*, which tells you which files the error touched. 
Here’s a sample stack trace from an error I saw earlier on:
...
This error is a `TypeError`, which means we used the wrong type of data for our operation 
(in this case, we tried to access a list as if it were a dictionary).

It’s really important that you get familiar with the most common errors you’ll see, 
so that as they appear in your programs you can quickly identify what went wrong.
...
As you go through the stack trace, it’s possible you’ll see some files and code that you didn’t write. 
This will happen if you use other modules and libraries (we’ll look at this later on!). 
If you see that, don’t worry! It’s unlikely those libraries will be the source of the error.

In many cases, the error won’t tell you everything you need to know in order to fix the problem. 
In that case, you’ll have to do some research:

1. Look at your code!
2. Put the error and message into Google, see if something comes up in StackOverflow.
3. Look at the code you’ve written again, this time more slowly, and run through it as if you were a computer. 
Do you notice anything that could potentially be a source of the error?
